...
// import { mergeDraftData, extractPriceMatrixOnly } from '../utils/formMerge';  // ← ADD THIS
...
//           geo: pincodeLookup.geo || {},
//           volumetric: volumetric.volumetric || volumetric.state || {},
//           charges: charges.charges || {},
//         };

//         // Smart merge: preserve user input over draft defaults
//         const merged = mergeDraftData(currentState, draft);

//         // Load merged data back into hooks
//         if (merged.basics && typeof vendorBasics.loadFromDraft === 'function') {
//           vendorBasics.loadFromDraft(merged.basics);
//           if (merged.basics.transportMode) setTransportMode(merged.basics.transportMode);
//         }
//         if (merged.geo && typeof pincodeLookup.loadFromDraft === 'function') {
//           pincodeLookup.loadFromDraft(merged.geo);
//         }
//         if (merged.volumetric && typeof volumetric.loadFromDraft === 'function') {
//           volumetric.loadFromDraft(merged.volumetric);
//         }
//         if (merged.charges && typeof charges.loadFromDraft === 'function') {
//           charges.loadFromDraft(merged.charges);
//         }
...
// const serviceModes = safeGetField(b, 'serviceModes');
// if (!serviceModes || (serviceModes !== 'Road LTL' && serviceModes !== 'Road FTL')) {
//   errs.push('Please select a valid service mode (Road LTL or Road FTL).');
// }

// // Validate company rating
// const rating = safeGetNumber(b, 4.0, 'companyRating');
// if (rating < 0 || rating > 5) {
//   errs.push('Company rating must be between 0 and 5.');
// }
...
//     const serviceModes = safeGetField(basics, 'serviceModes') || 'Road LTL';
//     const companyRating = safeGetNumber(basics, 4.0, 'companyRating');
...
//       fuel: parseCharge(safeGetNumber(c, 0, 'fuelSurcharge', 'fuel'), 0, 50, 2), // ← Keep this, it reads user input
...
//           safeGetNumber(c.handlingCharges || c, 0, 'threshholdweight', 'handlingThresholdWeight', 'weightThreshold'),  // ← Fixed field name
//           0, 100000
//         ),
//       },
//       appointmentCharges: {
...
//        // ✅ ONLY use priceMatrix from wizard, don't let wizard overwrite other fields
//     const wizardMatrix = extractPriceMatrixOnly(wizardData);
//     const priceChart = (wizardMatrix?.priceMatrix || zpm?.priceMatrix || {}) as PriceMatrix;
    
//     emitDebug('PRICE_MATRIX_SOURCE', {
//       fromWizard: !!wizardMatrix?.priceMatrix,
//       fromLegacy: !wizardMatrix?.priceMatrix && !!zpm?.priceMatrix,
//     });
...
//   serviceModes,              // ✅ NEW
//   companyRating,             // ✅ NEW
//   rating: companyRating,     // ✅ NEW (backend compatibility)
//   human: { name, primaryCompanyName, subVendor }, // ✅ REMOVED displayName
...
import { readDraft, clearDraft } from '../store/draftStore';
import { emitDebug, emitDebugError } from '../utils/debug';
import { mergeDraftData, extractPriceMatrixOnly } from '../utils/formMerge';
...
export const AddVendor: React.FC = () => {
  ...
  // ✅ FIXED: Load draft + zone matrix on mount with context-aware draft storage
  useEffect(() => {
    if (mountRan.current) return;
    mountRan.current = true;

    // ✅ CHANGE 1: Use context-aware draft reading
    const draft = readDraft('create'); // Only read 'create' context drafts
    if (draft) {
      emitDebug('DRAFT_LOADED_ON_MOUNT', draft);
      try {
        // Get current form state
  ...
  // ✅ FIXED: Clear unwanted hook defaults when there's no draft
  useEffect(() => {
    const draft = readDraft('create'); // Context-aware check
  ...
  const validateVendorBasicsLocal = (): { ok: boolean; errs: string[] } => {
    ...
    // ✅ ALIGNED WITH DOCUMENT 2: Field names match simple version
    // legalCompanyName → name/companyName (backend expects companyName)
    const legalCompanyName = capitalizeWords(safeGetField(b, 'legalCompanyName', 'name', 'companyName')).slice(0, 60);
    const companyName = capitalizeWords(safeGetField(b, 'companyName', 'primaryCompanyName')).slice(0, 25);
    const subVendor = capitalizeWords(safeGetField(b, 'subVendor', 'sub_vendor')).slice(0, 20);
    const vendorCode = sanitizeDigitsOnly(safeGetField(b, 'vendorCode', 'vendor_code')).slice(0, 9);

    // Primary Contact
    const primaryContactName = capitalizeWords(safeGetField(b, 'primaryContactName')).slice(0, 30);
    const primaryContactPhone = sanitizeDigitsOnly(safeGetField(b, 'primaryContactPhone')).slice(0, 10);
    const primaryContactEmail = safeGetField(b, 'primaryContactEmail').trim().slice(0, 20);

    // Vendor Contact
    const vendorPhone = sanitizeDigitsOnly(safeGetField(b, 'vendorPhone', 'vendorPhoneNumber')).slice(0, 10);
    const vendorEmail = safeGetField(b, 'vendorEmail', 'vendorEmailAddress').trim().slice(0, 20);
    
    const gstNo = safeGetField(b, 'gstNo', 'gstin', 'gst').toUpperCase().replace(/\s+/g, '').slice(0, 15);
    const address = safeGetField(b, 'address').trim().slice(0, 150);

    // Validations
    if (!legalCompanyName || legalCompanyName.trim().length === 0) {
      errs.push('Legal company name is required (max 60 chars).');
    }
    if (legalCompanyName.trim().length > 60) {
      errs.push('Legal company name must be at most 60 characters.');
    }
   
    if (companyName && companyName.trim().length > 25) {
      errs.push('Company name must be at most 25 characters.');
    }
    ...
    if (!primaryContactName || primaryContactName.trim().length === 0) {
      errs.push('Primary contact name is required (max 30 chars).');
    }
    
    if (!/^[1-9][0-9]{9}$/.test(primaryContactPhone)) {
      errs.push('Primary contact phone must be 10 digits and cannot start with 0.');
    }

    // ✅ Email validation with 20 char limit
    if (primaryContactEmail.length > 20) {
      errs.push('Primary contact email must be at most 20 characters.');
    }
    let primaryEmailOk = false;
    try { 
      primaryEmailOk = !!(primaryContactEmail && (isEmail.validate ? isEmail.validate(primaryContactEmail) : isEmail(primaryContactEmail))); 
    }
    catch { 
      primaryEmailOk = EMAIL_FALLBACK_RE.test(primaryContactEmail); 
    }
    if (!primaryEmailOk) {
      errs.push('Invalid primary contact email (must include a domain and a dot).');
    }

    // Vendor Contact Validations
    if (!/^[1-9][0-9]{9}$/.test(vendorPhone)) {
      errs.push('Vendor phone must be 10 digits and cannot start with 0.');
    }

    // ✅ Email validation with 20 char limit
    if (vendorEmail.length > 20) {
      errs.push('Vendor email must be at most 20 characters.');
    }
    let vendorEmailOk = false;
    try { 
      vendorEmailOk = !!(vendorEmail && (isEmail.validate ? isEmail.validate(vendorEmail) : isEmail(vendorEmail))); 
    }
    catch { 
      vendorEmailOk = EMAIL_FALLBACK_RE.test(vendorEmail); 
    }
    if (!vendorEmailOk) {
      errs.push('Invalid vendor email (must include a domain and a dot).');
    }
    ...
    try {
      const c = charges.charges || {};
      const fuel = safeGetNumber(c, 0, 'fuelSurcharge', 'fuel', 'fuelSurchargePercent');
      if (!Number.isFinite(fuel) || fuel < 0 || fuel > 50) {
        errs.push('Fuel surcharge must be between 0 and 50.');
      }
    } catch { /* ignore */ }
    ...
    // ✅ Service mode validation (aligned with document 2: road_ltl / road_ftl)
    const serviceMode = safeGetField(b, 'serviceMode', 'serviceModes');
    if (!serviceMode || (serviceMode !== 'road_ltl' && serviceMode !== 'road_ftl' && serviceMode !== 'Road LTL' && serviceMode !== 'Road FTL')) {
      errs.push('Please select a valid service mode (Road LTL or Road FTL).');
    }

    // ✅ Rating validation (aligned with document 2)
    const rating = safeGetNumber(b, 0, 'rating', 'companyRating');
    if (rating < 0 || rating > 5) {
      errs.push('Company rating must be between 0 and 5.');
    }
  ...
  // ===== Build API payload (aligned with document 2 structure) =====
  const buildPayloadForApi = () => {
    const basics = vendorBasics.basics || {};
    const geo = pincodeLookup.geo || {};

    // ✅ ALIGNED: Field names match document 2
    const legalCompanyName = capitalizeWords(safeGetField(basics, 'legalCompanyName', 'name', 'companyName')).slice(0, 60);
    const companyName = capitalizeWords(safeGetField(basics, 'companyName', 'primaryCompanyName')).slice(0, 25);
    const subVendor = capitalizeWords(safeGetField(basics, 'subVendor', 'sub_vendor')).slice(0, 20);
    const vendorCode = sanitizeDigitsOnly(safeGetField(basics, 'vendorCode', 'vendor_code')).slice(0, 9);

    // Primary Contact
    const primaryContactName = capitalizeWords(safeGetField(basics, 'primaryContactName')).slice(0, 30);
    const primaryContactPhoneStr = sanitizeDigitsOnly(safeGetField(basics, 'primaryContactPhone')).slice(0, 10);
    const primaryContactPhoneNum = Number(clampNumericString(primaryContactPhoneStr, 1000000000, 9999999999, 10) || 0);
    const primaryContactEmail = safeGetField(basics, 'primaryContactEmail').trim().slice(0, 20);

    // Vendor Contact
    const vendorPhoneStr = sanitizeDigitsOnly(safeGetField(basics, 'vendorPhone', 'vendorPhoneNumber')).slice(0, 10);
    const vendorPhoneNum = Number(clampNumericString(vendorPhoneStr, 1000000000, 9999999999, 10) || 0);
    const vendorEmail = safeGetField(basics, 'vendorEmail', 'vendorEmailAddress').trim().slice(0, 20);

    const gstNo = safeGetField(basics, 'gstNo', 'gstin', 'gst').toUpperCase().replace(/\s+/g, '').slice(0, 15);
    const address = safeGetField(basics, 'address').trim().slice(0, 150);

    // ✅ Service mode (support both formats)
    const serviceMode = safeGetField(basics, 'serviceMode', 'serviceModes') || 'road_ltl';
    const serviceModes = serviceMode === 'road_ltl' || serviceMode === 'Road LTL' ? 'Road LTL' : 'Road FTL';

    // ✅ Rating
    const rating = safeGetNumber(basics, 0, 'rating', 'companyRating');

    // Volumetric
    const volData = volumetric.volumetric || volumetric.state || (volumetric as any).data || {};
    const volumetricUnit = safeGetField(volData, 'volumetricUnit', 'unit', 'selectedUnit') || 'cm';
    const volumetricDivisor = safeGetNumber(volData, 0, 'volumetricDivisor', 'divisor') || null;

    emitDebug('VOLUMETRIC_DATA_DEBUG', { volData, volumetricUnit, volumetricDivisor });

    const volumetricBits =
      volumetricUnit === 'cm' || volumetricUnit === 'centimeters'
        ? { divisor: volumetricDivisor, cftFactor: null as number | null }
        : { divisor: null as number | null, cftFactor: safeGetNumber(volData, 0, 'cftFactor', 'factor') || null };
    ...
    // ✅ ALIGNED: Price rate structure matches document 2
    const priceRate = {
      minChargeableWeight: parseCharge(safeGetNumber(c, 0, 'minChargeableWeight', 'minWeight'), 0, 10000, 5),
      docketCharges: parseCharge(safeGetNumber(c, 0, 'docketCharges'), 0, 10000, 5),
      fuelSurchargePercent: parseCharge(safeGetNumber(c, 0, 'fuelSurchargePercent', 'fuelSurcharge', 'fuel'), 0, 50, 2),
      minimumCharges: parseCharge(safeGetNumber(c, 0, 'minimumCharges', 'minCharges'), 0, 100000),
      greenTax: parseCharge(safeGetNumber(c, 0, 'greenTax', 'ngt'), 0, 100000),
      daccCharges: parseCharge(safeGetNumber(c, 0, 'daccCharges'), 0, 10000),
      miscellaneousCharges: parseCharge(safeGetNumber(c, 0, 'miscellaneousCharges', 'miscCharges', 'miscellanousCharges'), 0, 100000),
      hamaliCharges: parseCharge(safeGetNumber(c, 0, 'hamaliCharges', 'handlingCharges'), 0, 10000),

      // ✅ ALIGNED: Special charges match document 2 structure
      handling: {
        unit: safeGetField(c.handlingCharges || c.handling || c, 'unit') || 'per_kg',
        fixed: parseCharge(safeGetNumber(c.handlingCharges || c.handling || c, 0, 'fixed', 'handlingFixed'), 0, 100000),
        variable: parseCharge(safeGetNumber(c.handlingCharges || c.handling || c, 0, 'variable', 'handlingVariable'), 0, 100000),
        weightThreshold: parseCharge(
          safeGetNumber(c.handlingCharges || c.handling || c, 0, 'weightThreshold', 'threshholdweight', 'handlingThresholdWeight'),
          0, 100000
        ),
      },

      rov: {
        fixed: parseCharge(safeGetNumber(c.rovCharges || c.rov || c, 0, 'fixed', 'rovFixed'), 0, 100000),
        variable: parseCharge(safeGetNumber(c.rovCharges || c.rov || c, 0, 'variable', 'rovVariable'), 0, 100000),
      },

      cod: {
        fixed: parseCharge(safeGetNumber(c.codCharges || c.cod || c, 0, 'fixed', 'codFixed'), 0, 100000),
        variable: parseCharge(safeGetNumber(c.codCharges || c.cod || c, 0, 'variable', 'codVariable'), 0, 100000),
      },

      toPay: {
        fixed: parseCharge(safeGetNumber(c.toPayCharges || c.toPay || c, 0, 'fixed', 'topayFixed'), 0, 100000),
        variable: parseCharge(safeGetNumber(c.toPayCharges || c.toPay || c, 0, 'variable', 'topayVariable'), 0, 100000),
      },

      appointment: {
        fixed: parseCharge(safeGetNumber(c.appointmentCharges || c.appointment || c, 0, 'fixed', 'appointmentFixed'), 0, 100000),
        variable: parseCharge(safeGetNumber(c.appointmentCharges || c.appointment || c, 0, 'variable', 'appointmentVariable'), 0, 100000),
      },

      ...volumetricBits,

      // Additional charges (keep for backward compatibility)
      insuaranceCharges: {
        variable: parseCharge(safeGetNumber(c.insuranceCharges || c.insuaranceCharges || c, 0, 'variable', 'insuranceVariable'), 0, 100000),
        fixed: parseCharge(safeGetNumber(c.insuranceCharges || c.insuaranceCharges || c, 0, 'fixed', 'insuranceFixed'), 0, 100000),
      },
    ...
    // ✅ ALIGNED: Payload structure matches document 2
    const payloadForApi = {
      customerID: getCustomerIDFromToken(),
      
      // Company fields
      legalCompanyName,      // ✅ NEW (from doc 2)
      companyName,           // ✅ Display name
      subVendor,
      vendorCode,
      
      // Primary Contact
      primaryContactName,    // ✅ NEW (from doc 2)
      primaryContactPhone: primaryContactPhoneNum,  // ✅ NEW (from doc 2)
      primaryContactEmail,   // ✅ NEW (from doc 2)
      ...
      // Transport
      mode: transportMode || 'road',
      transportMode: transportMode || 'road',  // ✅ Keep both for compatibility
      serviceMode: serviceModes,  // ✅ NEW (from doc 2): "Road LTL" or "Road FTL"
      
      // Rating
      rating,                // ✅ NEW (from doc 2)
      companyRating: rating, // ✅ Keep both for compatibility
      
      // Volumetric
      volumetricUnit,        // ✅ NEW (from doc 2)
      volumetricDivisor,     // ✅ NEW (from doc 2)
      ...
      // Human-readable data (for display)
      human: {
        legalCompanyName,
        companyName,
        subVendor,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fd = new FormData();
      fd.append('customerID', String(payloadForApi.customerID || ''));
      fd.append('legalCompanyName', payloadForApi.legalCompanyName);
      fd.append('companyName', payloadForApi.companyName);
      fd.append('subVendor', payloadForApi.subVendor);
      fd.append('vendorCode', payloadForApi.vendorCode);
      
      fd.append('primaryContactName', payloadForApi.primaryContactName);
      fd.append('primaryContactPhone', String(payloadForApi.primaryContactPhone));
      fd.append('primaryContactEmail', payloadForApi.primaryContactEmail);
      ...
      fd.append('volumetricUnit', payloadForApi.volumetricUnit);
      fd.append('volumetricDivisor', String(payloadForApi.volumetricDivisor || ''));
      ...
      // ✅ FIXED: Clear context-aware draft, reset form, clear wizard data AND legacy localStorage
      clearDraft('create'); // Only clear 'create' context draft
      clearWizard(); // Clear wizard storage
      localStorage.removeItem(ZPM_KEY); // Clear legacy localStorage
      try {
        if (typeof vendorBasics.reset === 'function') vendorBasics.reset();
  ...
  const handleReset = () => {
    ...
    setPriceChartFile(null);
    setTransportMode('road');
    clearDraft('create'); // ✅ FIXED: Context-aware clear
    toast.success('Form reset', { duration: 1200 });
  };
        ...
        <form id="add-vendor-form" onSubmit={handleSubmit} className="space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white/90 shadow-sm overflow-hidden">
            <div className="grid grid-cols-1 gap-0 divide-y divide-slate-200">
              ...
              <div className="p-6 md:p-8 bg-slate-50/60">
                <TransportSection volumetric={volumetric} />
              </div>
