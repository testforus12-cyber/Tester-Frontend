...
const BiddingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await axios.get<{ data: ActiveBid[] }>(`http://tester-backend-4nxc.onrender.com/api/bidding/user/${userId}`).then(res => setActiveBids(res.data.data)).catch(err => console.error('Failed to load active bids', err));
    };
    fetchData();
  }, [user]);
  ...
  const onSubmit: SubmitHandler<FormValues> = async data => {
    try {``
      ...
      const response = await axios.post('http://tester-backend-4nxc.onrender.com/api/bidding/addbid', payload);
  ...
  const getBiddingDetails = async () => {
    try {
      const response = await axios.get(`http://tester-backend-4nxc.onrender.com/api/bidding/details/${expandedBidId}`);
