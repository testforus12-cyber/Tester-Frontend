...
export const getWizardStatus = (wizardData: WizardDataV1 | null): WizardStatus => {
  ...
  if (hasZones) completionScore += 40; // Zones are critical
  if (hasPriceMatrix) completionScore += 40; // Price matrix is critical
  if (hasOtherCharges) completionScore += 15; // Other charges are important
  if (hasODA) completionScore += 5; // ODA is optional
