// src/components/PincodeAutocomplete.tsx
...
interface PincodeSuggestion {
  ...
  city: string;
  state: string;
  district: string; // mirrors city if you don't store district separately
}
...
interface PincodeAutocompleteProps {
  ...
  value: string;
  placeholder: string;
  error?: string | null; // external page-level error (e.g., from/to equality)
...
const PincodeAutocomplete: React.FC<PincodeAutocompleteProps> = ({
  ...
  const [touched, setTouched] = useState(false); // gate error UI
  const [isValid, setIsValid] = useState(false); // local validity for Enter/blur paths
  ...
  const fetchSuggestions = useCallback(
    (query: string) => {
      ...
      const mapped: PincodeSuggestion[] = rows.map((r: any) => ({
        pincode: r.pincode,
        city: r.city,
        state: r.state,
        district: r.city,
  ...
  const formatError = validatePincode(value); // computed; we gate its display with "touched"
  ...
  // bubble validity upward (prefer local isValid when set, else computed)
  useEffect(() => {
    const outgoingValid = isValid || Boolean(isPincodeValid);
    onValidationChange?.(outgoingValid);
  }, [isValid, isPincodeValid, onValidationChange]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (digitsOnly.length === 6) {
      // optional UX: hide as soon as full length is reached
      setIsOpen(false);
    }
    setSelectedIndex(-1);
  };
  ...
  const handleBlur = () => {
    // Defer so click on a suggestion registers first
    setTimeout(() => {
      setIsOpen(false);
      setSelectedIndex(-1);
      ...
      if (value.length === 6 && /^[1-9]\d{5}$/.test(value)) {
        const exactMatch = getByPincode(value);
        if (exactMatch) {
          setIsValid(true);
  ...
  const handleSuggestionSelect = (suggestion: PincodeSuggestion) => {
    ...
    // blur after DOM/state settle to guarantee dropdown closes
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      // 1) Highlighted suggestion → select
      if (isOpen && selectedIndex >= 0 && selectedIndex < suggestions.length) {
        handleSuggestionSelect(suggestions[selectedIndex]);
        return;
      }

      // 2) Full typed pincode → attempt auto-select
      if (value.length === 6 && /^[1-9]\d{5}$/.test(value)) {
        const match = getByPincode(value);
        if (match) {
          const suggestion: PincodeSuggestion = {
          ...
          setIsValid(false);
          setIsOpen(false);
          // blur after state updates commit
          requestAnimationFrame(() => inputRef.current?.blur());
        }
        return;
      }

      // 3) Otherwise just close + blur
      setIsOpen(false);
      requestAnimationFrame(() => inputRef.current?.blur());
      return;
    }
  ...
  // ----------- UI gates -----------
  const shouldShowError = touched; // “pause” before showing errors

  const inputHasErrorUI =
    shouldShowError &&
    (Boolean(error) || Boolean(formatError) || isInvalidPincode);
  ...
  return (
    <div className={className}>
          ...
          <input
            ...
            className={`block w-full py-2 pl-10 pr-10 bg-white border rounded-lg text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-1 transition ${
              inputHasErrorUI
                ...
                : isUserSelected && (isValid || isPincodeValid)
      ...
      {/* Messages respect the touched gate */}
      ...
      {/* Optional hint when typing but not a valid 6-digit yet */}
      {shouldShowError &&
        value &&
        !isValidFormat &&
        !isInvalidPincode &&
